...
void write_to_vtk(const struct conf_stru &c, int t)
{
	...
	for (int id = 0; id < n; id++)
		par << c.p[id].x << " " << c.p[id].y << "  " << 0 << std::endl;
	...
	for (int id = 0; id < n; id++)
		par << c.energy_local[id][0] << std::endl;
...
void init(struct conf_stru &c)
{
	...
	for (int i = 0; i < c.p.size(); ++i)
	{
		file_neigbor_list << std::scientific << std::setprecision(16) << i << " " << c.p[i].nn[0] << " " << c.p[i].nn[1] << " " << c.p[i].nn[2] << " " << c.p[i].nn[3] << endl;
	}
	...
	for (const auto &element : c.myVector)
	{
		...
		file_triangles << std::scientific << std::setprecision(16) << i << " " << p.nn[k] << " " << p.nn[k + 1] << " " << endl;
	}
...
void memory(alglib::real_1d_array &x, struct conf_stru &c, double &load_value)
{
	...
	for (int i = 0; i < c.p.size(); ++i)
	{
		disp >> std::scientific >> std::setprecision(16) >> c.p[i].x >> c.p[i].y >> temp >> temp >> temp >> temp >> temp >> temp >> temp >> temp >> temp >> load_value;
	}
...
int main(int argc, char **argv)
{
	...
	if (method == 2)
	{
		...
		int mm;
		if (singleton.c.linearity == true)
			mm = starting_point_keep.length() / 2;
		if (singleton.c.linearity == false)
			mm = starting_point_keep.length();
