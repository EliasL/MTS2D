#!/usr/bin/env python3
"""Compare spatially averaged Ftotal and T components across VTU frames."""
...
def main() -> None:
    ...
    for path in files:
        ...
        for component in COMPONENTS:
            for matrix_name in ("Ftotal", "T"):
                field_name = f"{matrix_name}{component}"
                row[field_name] = float(np.mean(scalar_cell_field(mesh, field_name)))
        rows.append(row)
    ...
    for ax, component in zip(axes.flat, COMPONENTS):
        ...
        t_value = np.asarray([row[f"T{component}"] for row in rows])
        ax.plot(
            gamma,
            ftotal,
            color="#1769aa",
        ...
        ax.plot(
            ...
            marker="s",
            markersize=2.7,
            label=rf"$\langle T_{{{component}}}\rangle$",
        )
    ...
    fig.suptitle(
        r"Reconstructed full deformation $\mathcal{F}$ vs. plastic history $T$",
        fontsize=15,
    )
    fig.tight_layout()
