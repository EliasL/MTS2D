 ...
 * 	\brief Molecular dynamics integrators to find updated particle
 *positions.
 *
...
///
/// Molecular dynamics integrators to find updated system vectors. Mainly for
/// internal use.
///
template <typename Scalar> class MolecularDynamics {
...
public:
  ...
  ///			with the gradient.
  /// \param fx 		IN: The objective function value at the current
  /// point.
  ...
  static void VelocityVerlet(Foo &f, Scalar &fx, Vector &x, Vector &v,
                             Vector &grad, const Scalar &dt,
                             const Scalar &alpha, const Vector &m,
                             const FIREParam<Scalar> &param, void *optPtr) {
    ...
    case FIRE_NO_BOUNDARY_CONDITIONS:
      ...
      VelocityVerlet_NBC(f, fx, x, v, grad, dt, alpha, m, param, optPtr);
      break;

    case FIRE_PERIODIC_BOUNDARY_CONDITIONS:

      VelocityVerlet_PBC(f, fx, x, v, grad, dt, alpha, m, param, optPtr);
      break;
  ...
  template <typename Foo>
  static void VelocityVerlet_NBC(Foo &f, Scalar &fx, Vector &x, Vector &v,
                                 Vector &grad, const Scalar &dt,
                                 const Scalar &alpha, const Vector &m,
                                 const FIREParam<Scalar> &param, void *optPtr) {
  ...
  template <typename Foo>
  static void VelocityVerlet_PBC(Foo &f, Scalar &fx, Vector &x, Vector &v,
                                 Vector &grad, const Scalar &dt,
                                 const Scalar &alpha, const Vector &m,
                                 const FIREParam<Scalar> &param, void *optPtr) {
    ...
    for (int i = 0; i < param.n_p; i++) {
      for (int j = 0; j < param.dim; j++) {
        ...
        x(i + j * param.n_p) -=
            len(j) * std::floor((x(i + j * param.n_p) - lbnd(j)) / len(j));
      }
    }
  ...
  template <typename Foo>
  static void VelocityVerlet_HBC(Foo &f, Scalar &fx, Vector &x, Vector &v,
                                 Vector &grad, const Scalar &dt,
                                 const Scalar &alpha, const Vector &m,
                                 const FIREParam<Scalar> &param, void *optPtr) {
